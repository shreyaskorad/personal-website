...
def parse_citation_policy(payload: dict[str, Any]) -> dict[str, Any]:
    ...
    if isinstance(recent_domains_raw, (list, tuple, set)):
        for domain in recent_domains_raw:
            value = sanitize_text(domain)
            if value:
                recent_domains.add(value.lower())

    return {
        'target_count': min(CITATION_MAX_COUNT, max(0, target_count)),
...
def gather_live_research(title: str, lead: str, tags: list[str]) -> dict[str, Any]:
    ...
    for item in ranked:
        ...
        if not re.search(r'\b(work|workplace|team|manager|learning|training|coaching|performance|decision|behavior|behaviour|analytics|capability)\b', title_l):
            continue
        phrase_words = [w for w in re.findall(r"[A-Za-z][A-Za-z0-9&'-]*", title_text) if w]
        phrase_words = [w for w in phrase_words if w.lower() not in {'and', 'or', 'for', 'with', 'to', 'of', 'in'}]
        phrase = ' '.join(phrase_words[:7]).strip(' -:;,.')
        if not phrase:
            continue
        candidate = (
...
def select_optional_citations(payload: dict[str, Any]) -> dict[str, Any]:
    ...
    required_count = int(citation_policy.get('target_count', 0) or 0)
    required_new_domains = int(citation_policy.get('required_new_domains', 0) or 0)
    prior_domains = {sanitize_text(d).lower() for d in citation_policy.get('recent_domains', set()) if sanitize_text(d)}
    required_count = max(0, min(OPTIONAL_CITATION_MAX, required_count))
    ...
    if len(citations) < required_count:
        fallback = ensure_study_citations(
...
def quality_report(payload: dict[str, Any]) -> dict[str, Any]:
    ...
    if has_cadence:
        evidence += 1
    if citation_count >= required_citation_count and required_citation_count > 0:
        evidence += 1
    if citation_count >= 1:
        evidence += 1
    if new_domain_count >= required_new_domain_count and required_new_domain_count > 0:
        evidence += 1
    if topic_relevant_citation_count >= max(1, min(citation_count, required_citation_count)):
        evidence += 1
    if citation_confidence >= 0.78:
        evidence += 1
...
def build_quality_critique(report: dict[str, Any], target_total: int) -> list[str]:
    ...
    for dim in ordered:
        ...
        floor = int(QUALITY_DIMENSION_FLOORS.get(dim, 4))
        if score >= floor:
            continue
        if dim == 'clarity':
...
def hard_quality_failures(report: dict[str, Any]) -> list[str]:
    ...
    citation_confidence = float(signals.get('citation_confidence') or 0.0)
    generic_citation_titles = int(signals.get('generic_citation_titles') or 0)

    if citation_count < required_citation_count:
        failures.append(f'citation count {citation_count} is below required minimum {required_citation_count}')
    if required_new_domain_count > 0 and citation_domain_count < required_new_domain_count:
        failures.append(
            f'new-domain diversity count {citation_domain_count} is below minimum required {required_new_domain_count}'
        )
    if required_citation_count > 0 and topic_relevant_citation_count < 1:
        failures.append('citation set has no clearly topic-relevant sources')
    if required_citation_count > 0 and citation_count > 0 and generic_citation_titles >= citation_count:
        failures.append('citation entries are generic placeholders and do not provide clear source titles')
    if required_citation_count > 0 and citation_confidence < 0.48:
        failures.append(f'citation confidence {citation_confidence:.2f} is below minimum 0.48')
...
def dimension_quality_failures(report: dict[str, Any]) -> list[str]:
    failures: list[str] = []
    scores = report.get('scores', {}) if isinstance(report.get('scores', {}), dict) else {}
    for dim, floor in QUALITY_DIMENSION_FLOORS.items():
        score_payload = scores.get(dim, {}) if isinstance(scores.get(dim, {}), dict) else {}
        score = int(score_payload.get('score') or 0)
        if score < int(floor):
            failures.append(f'{dim} score {score} is below minimum {int(floor)}')
    return failures
...
def simplify_sentence(text: str, max_words: int = 24) -> str:
    ...
    def finalize(value: str) -> str:
        candidate = sanitize_content_line(value)
        if candidate:
            return candidate
        repaired = sanitize_text(value).rstrip(',;:')
        if repaired and repaired[-1] not in '.!?':
            repaired += '.'
        if is_likely_sentence_fragment(repaired):
            repaired = repaired.rstrip('.').rstrip() + ' in real workflows.'
        return sanitize_content_line(repaired) or repaired
...
def quality_targets(slug: str, min_total: int, delta: int) -> tuple[int, int, dict[str, Any]]:
    ...
    global_best = min_total
    if isinstance(by_slug, dict):
        for item in by_slug.values():
            if not isinstance(item, dict):
                continue
            previous_total = item.get('last_total')
            try:
                previous_total = int(previous_total)
            except Exception:
                continue
            if previous_total > global_best:
                global_best = previous_total

    effective_previous = max(previous, global_best)
    ratchet_cap = 25 if min_total >= 25 else min(25, min_total + delta)
    target = max(min_total, min(ratchet_cap, effective_previous + (delta if effective_previous > 0 else 0)))
...
def run_quality_gate(
    ...
    for index in range(1, attempts + 1):
        ...
        report['hard_failures'] = ordered_unique([*hard_quality_failures(report), *dimension_quality_failures(report)])
...
def normalize_publish_title(raw_title: Any, raw_description: Any) -> str:
    ...
    def invalid_title(candidate: str) -> bool:
        ...
        return (
            ...
            or re.search(r'\(\s*\d{3,}\s*\)\s*$', value_l) is not None
        )
...
def sanitize_payload(raw: dict[str, Any]) -> dict[str, Any]:
    ...
    citations = merge_citations(raw.get('citations', []), raw.get('_citations', []))
