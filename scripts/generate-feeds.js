...
const sitemapUrls = [
  ...
  ...articles.map((article) => ({
    loc: `${baseUrl}/${article.href}`.replace(/\/+/g, '/'),
...
const imageEntries = articles.filter((article) => article.image).map((article) => {
  const loc = `${baseUrl}/${article.href}`.replace(/\/+/g, '/');
  const imageLoc = article.image.startsWith('http') ? article.image : `${baseUrl}/${article.image}`.replace(/\/+/g, '/');
  return {
    loc,
    imageLoc,
    title: article.title,
...
const rssItems = articles.map((article) => {
  const url = `${baseUrl}/${article.href}`.replace(/\/+/g, '/');
